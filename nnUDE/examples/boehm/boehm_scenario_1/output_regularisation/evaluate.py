...
from models import get_unregularised_problems

pypesto_importers, pypesto_problems = get_unregularised_problems()
...
calculate_additional_metrics(
    ...
    importers=(
        pypesto_importers[UDE],
        pypesto_importers[NUDE],
        pypesto_importers[BNUDE],
    ),
    fp=fp_losses,
)
...
paper_figure_nonneg_runningtime(
    df_losses, LABELS, fp=result_path / "tradeoff.png",
)
...
# best fit
for label, result in zip(LABELS, (ude_result, nude_result, bnude_result)):
    pypesto_importer = pypesto_importers[label]
    # no issue with using the problem with regularised objective here
    # because I'm not using the fval
...
for n in [20, 50]:
    ...
    plot_ensembles(
        ...
        pypesto_problems=pypesto_problems,
        labels=LABELS,
        n=n,
        fp=fp,
    )
...
compare_state_trajectories_boehm(
    ...
    results=(ude_result, nude_result, bnude_result),
    pypesto_problems=(
        pypesto_problems[UDE],
        pypesto_problems[NUDE],
        pypesto_problems[BNUDE],
    ),
    pypesto_result_id=0,
